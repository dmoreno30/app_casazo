...
class getDataDealQuoteController extends \Leaf\Controller
{
    ...
    public function index()
    {
        ...
        $this->DealBitrix->MessaggeDeal($id, "Mensaje de Sistema: Información enviada a nova, espere por favor...");
        $resultNova = $this->NovaQuote->findQuote($id, $companyCode);
        $this->helpers->LogRegister($resultNova, "resultNova create ");


        switch ($resultNova["statusCode"]) {
            case '200':
                $this->DealBitrix->MessaggeDeal($id, "Mensaje de Sistema: Cotización existente, en proceso de actualización en Nova");
                ...
                switch ($resultUpdateQuote["statusCode"]) {
                    case 200:
                        $this->DealBitrix->MessaggeDeal($id, "Mensaje de Sistema: Información de NOVA - {$resultUpdateQuote['message']}");
                        break;
                    case 400:
                        $this->DealBitrix->MessaggeDeal($id, "Mensaje de Sistema: Información de NOVA - {$resultUpdateQuote['message']}");
                        break;
            ...
            case '400':
                ...
                $this->helpers->LogRegister($data, "buildDataFromBitrix data");
    ...
    private function buildDataFromBitrix($resultBitrix)
    {
        ...
        try {
            ...
            $fechaInicio = $this->helpers->FormatDate($resultBitrix["BEGINDATE"]);
            $fechaFinal = $this->helpers->FormatDate($resultBitrix["CLOSEDATE"]);
            return [
                [
                    ...
                    "fechaDoc" =>  (string)$fechaInicio,
                    "fechaVencimientoDoc" => (string)$fechaFinal,
            ...
            $this->helpers->LogRegister("Error Catch",$th->getMessage());
        }
    }
